...
- `POST /api/actions/:actionId` body `{ input, mode: "plan" | "execute", scope: { projectId?, environmentId? }, idempotencyKey? }` → `{ plan?, result?, idempotency? }` — thin wrapper over `runAction`; actor derived from the demo session (single local user "you"). **Idempotency**: a key is scoped by workspace, actor and action and bound to a canonical hash of `{ input, projectId, environmentId }`. Same key + same request → the first request's **retained outcome** (`idempotency.replayed: true`); same key + a different request → `idempotency_conflict`. An outcome is retained only *after* that request's durable write has succeeded, so `replayed: true` is evidence the mutation was committed — a retry that lands while the first request is still committing is refused as `idempotency_in_flight` and should simply be retried. The window is process-local and best-effort (`idempotency: { applied, scope: "process", bestEffort: true, replayed, windowMs, note }`): a restart clears it and another instance does not share it, so a retry that crosses either runs the action again.
