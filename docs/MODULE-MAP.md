...
| `src/lib/db/postgres-store.ts` | The second implementation of `Store`: Supabase/PostgREST over a **request-scoped snapshot** (`route()` prefetches the caller's workspace slice before the handler runs), row-level optimistic concurrency (`version` guard; 0 rows updated → `ApiError` 409 "Someone else changed this workspace; reload and retry"), and a `workspace_versions` poll (300 ms) in place of the in-process change emitter. **Hybrid**: Phase-3 method groups delegate to `FileStore` through `db/pg/delegates.ts`. Holds no per-collection knowledge itself — it iterates `db/pg/registry.ts` | L0–L1, `db/{types,file-store,request-snapshot,pg/*}`, `supabase/admin` |
| `src/lib/db/pg/` | The Postgres store's extension points, so four agents can extend it without editing one file: `registry.ts` (the `CollectionAdapter` contract, `registerCollection()`/`adapters()`, and the tenant/hydration helpers — registration order **is** foreign-key order), `core.ts` (the Phase-2 collections registering themselves) and `delegates.ts` (the mutable `delegates` object + `setDelegate()` for the method groups still answered by `FileStore`: `manifests`, `events`, `audit`). Secrets are absent on purpose — they never went through `Store` | L0–L1, `db/{types,file-store}`, `domain/types` |
