...
| Verified in this repo | The platform recipe and module-origin boundary are contract-tested; toolchain pinned in `package.json`: `vite@7.3.6`, `@vitejs/plugin-react@5.1.4`. | `e2b@2.46.1` is installed. The adapter requires a bare `ZENITH_E2B_TEMPLATE` ID and matching `ZENITH_E2B_TEMPLATE_DIGEST`, verifies the provider-resolved ID and pre-baked attestation, performs no job-time package install, checks pinned recipe versions, disables sandbox internet access, and always tears down; contract-tested with an injected SDK double. **No sandbox has ever been created.** | Not verified. |
