...
| E2B dependency supply chain and egress | **Local policy implemented; live hold** | `runner-e2b.ts` requires a bare immutable template ID plus matching SHA-256 attestation, verifies the provider-resolved ID before upload, performs no job-time package install, checks pinned versions, disables sandbox internet access, and tears down; provider parity remains unverified. | D with F | Frozen-template/attestation/install-policy tests, egress/teardown evidence, disposable live lane |
