...
| Synchronous PostgREST bridge | **Reproduced; rework** | `src/lib/db/pg/sync-rest.ts` uses a worker and `Atomics.wait` for up to 15 seconds; used by history/audit/secrets paths. The scoped async repository now binds tenant write bodies. | A | Async API contract and tenant-boundary tests passed; secret callers and event-loop measurement remain |
