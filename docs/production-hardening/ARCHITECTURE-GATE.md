...
| E2B dependency supply chain and egress | **Local policy implemented; live hold** | `runner-e2b.ts` requires a bare immutable template ID, matching SHA-256 attestation, and trusted Ed25519 signature, verifies the provider-resolved ID before upload, performs no job-time package install, checks pinned versions, disables sandbox internet access, and tears down; provider parity remains unverified. | D with F | Frozen-template/attestation/install-policy tests, egress/teardown evidence, disposable live lane |
| Synchronous PostgREST bridge | **Substantially widened; rework remains** | `sync-rest.ts` still uses a worker and `Atomics.wait` for legacy Store/history compatibility; secret route/actions/provider/alerts, cold readers, deploy planning, and action audit writes now use awaited storage and the async repository binds tenant bodies. | A | Async API contract, secret caller, cold-reader, audit, and tenant-boundary tests passed; legacy history contract widening and event-loop measurement remain |
