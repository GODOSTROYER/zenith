...
Heading: **Your cloud. / Clear boundaries.** Registry-fed provider rows show the current connection limits. Sandbox simulates without provisioning. LocalStack supports local S3 buckets and SQS queues, with services/routes/unsupported configurations blocked by preflight. AWS Preview exports real Terraform without in-app apply, account reads or live account verification. Kubernetes and Google Cloud appear under **On the horizon** as **Planned**; Microsoft Azure and the explicit Oracle roadmap entry say **Coming later**. None is presented as a working connection, and their provider IDs/status values are unchanged.
