...
export function LandingClose({ cta }: { cta: Cta }) {
  return (
    <section className="zenith-close" aria-labelledby="zenith-close-title">
      ...
      <footer className="zenith-footer"><a href="#main" aria-label="Zenith home"><Wordmark size={26} /></a><p>The next change, made tangible.</p><nav aria-label="Footer navigation"><Link href="/login">Sign in</Link><Link href="/guide">Guide</Link><a href="https://github.com/GODOSTROYER/zenith" target="_blank" rel="noreferrer">GitHub <ArrowUpRight size={13} aria-hidden="true" /><span className="sr-only"> (opens in a new tab)</span></a></nav></footer>
    </section>
  );
}
