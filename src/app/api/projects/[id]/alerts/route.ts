...
const deliveryNote = (enabled: number): string =>
  enabled === 0
    ? "In-product only. This workspace has no enabled delivery channels, so an alert is seen when somebody opens Zenith. Add one under Settings → Alerts."
...
export const GET = route<{ id: string }>(async (req, { id }) => {
  ...
  return {
    ...
    recent,
    /**
     * True: every condition Zenith can evaluate today reads generated health or
     * estimated cost. Individual events carry their own `simulated` flag — a
     * deployment failure on a real adapter is a real record.
     */
    simulated: true,
    generatedBy:
      "Zenith's alert evaluator, over the log simulator's health and the price table's estimates",
