 ...
 *    `X-Orrery-Signature: sha256=<hex>`, so a receiver can prove Orrery sent it.
...
export const emailBody = (msg: AlertMessage): { subject: string; text: string } => ({
  ...
  text: [
    ...
    "\nSent by Orrery. Reply-to is not monitored — acknowledge the alert in Observe → Alerts.",
  ]
    .filter(Boolean)
    .join("\n"),
});
...
export const MISSING_NODEMAILER =
  "The nodemailer package is not installed, so email alerts cannot be sent. Run `npm install` in the Orrery repo (nodemailer is in package.json) and restart the server. Webhook and Slack channels do not need it.";
