...
| `sessions.ts` | The 60-second exchange code, the opaque app session behind it, and the `__Host-zenith_app` cookie. Neither value is stored — only SHA-256 of each. `resolveAppSession` re-reads session and grant every time | Let a platform cookie reach an app host; allow a code presented against the wrong app or state to be replayed (a mismatch consumes and stays consumed). `ponytail:` — nothing sweeps expired exchange and session rows; they are denied on read |
| `identity.ts` | The authoritative check: a live `auth.getUser()` round trip, required before anything that grants, revokes, invites, accepts or opens an app (R3-10, G13) | Return an identity it did not get from the provider. **Unavailable is never a pass**: no provider, a network error, a 5xx or a rate limit all answer `policy_unavailable` (503); only a positive "not signed in" answers 401. `ponytail:` — one round trip per grant-sensitive request, deliberately uncached |
