...
| `seal.ts` | AES-256-GCM over the invitation token, under `ZENITH_SECRET_KEY`, for as long as one delivery row is outstanding. The invitation id is the AAD | Import `@/lib/secrets` — that store is a JSON file of workspace secrets and has no business being reachable from app access |
