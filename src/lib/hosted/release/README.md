...
The control authority is asynchronous, so almost everything here is: `tickJobs`,
`buildSlot`, `claimJob`, `admitPublish`/`admitRollback`/`admitSuspend`, the
`runPublish`/`runRollback`/`runSuspend` pipelines and the `shared.ts` plumbing
(`requireApp`, `advanceTo`, `persist`, `failJob`, `emit`) all return Promises.
Inside a `tx()` the callback's own `repos` is what the code uses — `requireApp`
takes them as an optional second argument for exactly that — because reaching
for `authority().repos` there is a second connection on anything but SQLite.
`renewLease` is the one deliberate exception: the heartbeat fires from a
`setInterval`, which has nowhere to await.
...
| `phases/stage.ts` | The candidate release row (committed with the job's knowledge of it) and the runtime's staging call | Re-read the active fence. The fence compared at activation is the one read here. `TODO(ceiling):` — raw SQL for the staged runtime ref, since `ReleasesRepo` has no setter; the third and last `sqliteConnection()` call in this directory |
