...
| `phases/stage.ts` | The candidate release row (committed with the job's knowledge of it) and the runtime's staging call | Re-read the active fence. The fence compared at activation is the one read here. `ponytail:` — raw SQL for the staged runtime ref, since `ReleasesRepo` has no setter |
