 ...
 * Zenith's principal with an ExternalId, and Zenith would assume it for
...
const PREVIEW_ACCESS = [
  "No AWS access at all. No role to create, no key to grant, no trust to extend — Zenith makes no AWS API call in Preview",
  "No credentials are stored or transmitted. AWS_ACCESS_KEY_ID on the Zenith server, if set, is reported as present and is never used to call AWS",
...
function planSteps(env: Environment, next: Manifest, previous?: Manifest): ProviderPlanStep[] {
  ...
  // it names no role for Zenith to assume: there is no cross-account trust in
      ...
      checks: [
        {
          ...
          status: "warn",
          detail: `A value is present in this server's environment (region ${region}). Zenith has not called AWS with it, so this says nothing about whether the key is valid or what it can reach.`,
        ...
        },
        {
          ...
          status: "warn",
          detail:
            "Credentials detected but apply is disabled in Preview. Zenith will plan and export, never mutate your account.",
    ...
    checks: [
      {
        ...
        // would unlock, and there is none — nothing in Zenith reads AWS today.
        id: "aws.credentials",
        label: "No AWS credentials on this server",
        status: "warn",
        detail:
          "Zenith never calls AWS in Preview, so nothing here needs credentials. Setting them would not unlock reads, plans or applies today.",
        fix: "Export the Terraform from Settings → Export and run it with your own credentials. Setting AWS_ACCESS_KEY_ID / AWS_SECRET_ACCESS_KEY on the Zenith server changes nothing today: the AWS provider plans and exports only, and no code path reads your account.",
      },
      {
        id: "aws.apply",
        label: "Apply is disabled in Preview",
...
export const awsProvider: ProviderAdapter = {
  ...
  accessExplanation: () => ({
    summary:
      "This connection grants Zenith no access to your AWS account, because Zenith does not call AWS in Preview. Create no IAM role and extend no trust: there is nothing here to use it. Zenith plans your system and generates real Terraform from your manifest, on this server; you run that Terraform with your own credentials, and that is the only way anything reaches your account. When a read path exists, the exact permission it needs will be listed here — and not one line before it works.",
    permissions: PREVIEW_ACCESS,
  }),
