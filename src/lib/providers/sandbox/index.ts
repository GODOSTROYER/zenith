 ...
 * stream is invented-but-credible Orrery Sandbox output; it never impersonates
...
async function executeStep(rt: StepRuntime): Promise<void> {
  ...
  rt.log(`Simulated deployment complete — this is the Orrery Sandbox, not real cloud infrastructure.`, "info");
}
    ...
    files: [
      {
        ...
        content: `# Generated by Orrery from ${env.name} — a local stand-in for the sandbox.\nservices:\n${services}${resources}`,
      },
...
- \`orrery.manifest.json\` — the canonical Orrery model. Every service,
...
with your own credentials, with or without Orrery.
`,
  };
}
...
export const sandboxProvider: ProviderAdapter = {
  ...
  accessExplanation: () => ({
    summary:
      "The sandbox runs entirely inside Orrery. It never touches a cloud account and needs no credentials.",
