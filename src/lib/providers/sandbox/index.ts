 ...
 * stream is invented-but-credible Zenith.ai Sandbox output; it never impersonates
...
function injectSecrets(rt: StepRuntime, service: Service): void {
  ...
  for (const e of refs) {
    ...
    if (!ref.startsWith("vault:")) {
      rt.log(`inject ${e.key} ← ${ref} (your secret manager; Zenith.ai does not resolve it)`, "provider");
      continue;
    }
    ...
    if (held.exists)
      rt.log(`inject ${e.key} ← ${ref} (v${held.version}, from the Zenith.ai secret store)`, "provider");
...
async function executeStep(rt: StepRuntime): Promise<void> {
  ...
  if (step.phase === "provision" && resource) {
    ...
    if (resource.ownership !== "managed") {
      ...
      rt.log(
        `${resource.name} is referenced, not managed — Zenith.ai reads it and never mutates it.`,
        "info"
      );
      return;
    }
  ...
  rt.log(`Simulated deployment complete — this is the Zenith.ai Sandbox, not real cloud infrastructure.`, "info");
}
    ...
    files: [
      {
        ...
        content: `# Generated by Zenith.ai from ${env.name} — a local stand-in for the sandbox.\nservices:\n${services}${resources}`,
      },
...
- \`orrery.manifest.json\` — the canonical Zenith.ai model. Every service,
...
with your own credentials, with or without Zenith.ai.
`,
  };
}
...
export const sandboxProvider: ProviderAdapter = {
  ...
  accessExplanation: () => ({
    summary:
      "The sandbox runs entirely inside Zenith.ai. It never touches a cloud account and needs no credentials.",
