...
type ChaosGlobals = typeof globalThis & { __zenithChaosBurned?: Set<string> };

function burned(): Set<string> {
  const g = globalThis as ChaosGlobals;
  if (!g.__zenithChaosBurned) g.__zenithChaosBurned = new Set();
  return g.__zenithChaosBurned;
}

/** `ZENITH_CHAOS` on a service: "fail_once" | "degrade". */
export function chaosFlag(s: Service): string | undefined {
  return s.env.find((e) => e.key === "ZENITH_CHAOS")?.value;
}
...
function exportBundle(env: Environment, manifest: Manifest): ExportBundle {
  ...
  return {
    files: [
      ...
      { path: "zenith.manifest.json", content: JSON.stringify(manifest, null, 2) },
    ],
...
- \`zenith.manifest.json\` — the canonical Zenith model. Every service,
...
function driftedValue(v: string): string {
  const n = Number(v);
  return Number.isFinite(n) && v.trim() !== "" ? String(n + 1) : `${v}-changed-outside-zenith`;
}
