...
// Minimal stand-in for the real file authority (src/lib/agent-access/authority/file.ts,
// P1's, not yet present in this worktree): verify() = authenticate(header, loadCredentials(file)),
// re-read every call — the "no behaviour change on the authorization path" rule of
// LINK-PROTOCOL.md §3.1.
function fileAuthority(file,kind='file'){return {kind,async ready(){await loadCredentials(file);},async verify(header){return authenticate(header,await loadCredentials(file));}};}
function handler(file,more={}){const {enabled,authority,...rest}=more;return createReaderHandler({enabled:enabled??(async()=>true),authority:authority??(()=>fileAuthority(file)),origin:'http://127.0.0.1:3400',credentialsPath:file,tools:[{name:'zenith_get_context',description:'fixture',scope:'read',inputSchema:{type:'object'}},{name:'zenith_plan_deploy',description:'fixture',scope:'plan',inputSchema:{type:'object'}}],inScope:async(_g,_s,fn)=>fn(),call:async()=>({value:'private',workspaceId:'ws'}),...rest});}
...
test('disabled, non-loopback and foreign-origin profiles refuse',async()=>fixture(async file=>{assert.equal((await handler(file,{enabled:async()=>false})(req(message))).status,503);assert.equal((await handler(file,{origin:'https://example.com'})(req(message))).status,503);assert.equal((await handler(file)(req(message,{origin:'https://foreign.example'}))).status,403);}));
test('a configured HTTPS origin is accepted only when the authority is postgres',async()=>fixture(async file=>{assert.equal((await handler(file,{origin:'https://agent.zenith.test',authority:()=>fileAuthority(file,'postgres')})(req(message,{host:'agent.zenith.test'}))).status,200);}));
