...
const app = seedApp(a, { slug: "score-app", workspaceId: "ws-score", createdBy: BUILDER });
...
function event(
  ...
  a.tx(() =>
    a.repos.events.append({
      ...
      subjectHash: opts.subject ? subjectHash(opts.subject) : undefined,
...
for (const n of [1, 2, 3, 4]) event("invite.sent", { subject: BUILDER, ts: at(25 * DAY), logicalId: `invite-${n}` });
event("invite.accepted", { subject: COLLEAGUE, ts: at(20 * DAY + 5 * 60_000) });
event("invite.accepted", { subject: NEWCOMER, ts: at(3 * DAY + 2 * 60_000) });
event("invite.accepted", { subject: LURKER, ts: at(10 * DAY) });

// The colleague writes on day 0 of their cohort and again on day 7.
event("record.created", { subject: COLLEAGUE, ts: at(20 * DAY) });
event("record.updated", { subject: COLLEAGUE, ts: at(13 * DAY) });
// The newcomer writes two minutes after accepting; their cohort is 3 days old.
event("record.created", { subject: NEWCOMER, ts: at(3 * DAY) });
// The builder's own writes in their own app never activate it.
event("record.created", { subject: BUILDER, ts: at(21 * DAY) });
// Founder and test actors do the same work and are excluded from all of it.
event("record.created", { subject: FOUNDER, actorClass: "founder", ts: at(5 * DAY) });
event("record.updated", { subject: TESTER, actorClass: "test", ts: at(5 * DAY), assisted: true });

// Four publish jobs started, three reached an active release.
for (const n of [1, 2, 3, 4]) event("build.started", { subject: BUILDER, ts: at(24 * DAY), logicalId: `job-${n}` });
for (const n of [1, 2, 3]) event("release.activated", { subject: BUILDER, ts: at(24 * DAY), logicalId: `job-${n}` });
// A test actor's publish is excluded from the rate entirely.
event("build.started", { subject: TESTER, actorClass: "test", ts: at(24 * DAY), logicalId: "job-test" });
...
describe("scorecard", () => {
  it("counts a workspace as activated only for a write by someone other than the builder", () => {
    const metric = card().metrics.teamActivation;
  ...
  it("reports publish success as activated jobs over started jobs, excluding test actors", () => {
    const metric = card().metrics.publishSuccess;
    expect(metric.detail).toMatchObject({ started: 4, activated: 3 });
    expect(metric.value).toBeCloseTo(0.75, 10);
    expect(metric.n).toBe(4);
  });

  it("measures recipient conversion against invitations sent, not delivered", () => {
    const metric = card().metrics.recipientConversion;
  ...
  it("takes the median time from acceptance to a first write", () => {
    const metric = card().metrics.timeToUsefulAction;
  ...
  it("judges next-week return only on cohorts old enough to have had one", () => {
    const metric = card().metrics.nextWeekReturn;
    expect(metric.detail).toMatchObject({ cohort: 1, immature: 1, returned: 1 });
    expect(metric.value).toBe(1);
    expect(metric.n).toBe(1);
  });

  it("says unknown, with a reason, when every cohort is too young", () => {
    // A window whose end is right after the newcomer's only write: nobody in
    // it has had the chance to come back.
    const young = scorecard({ since: at(4 * DAY), until: at(3 * DAY - 60_000) });
  ...
  it("counts founder assistance and admits the minutes are not instrumented", () => {
    const metric = card().metrics.founderAssistance;
    expect(metric.value).toBe(1);
    expect(metric.detail.minutes).toBe("unknown");
    expect(metric.definition).toMatch(/minutes spent are not instrumented/);
  });

  it("reports how many events each class of actor contributed", () => {
    const actors = card().actors;
  ...
  it("reports every per-person measure as unknown when there is no salt", () => {
  ...
  it("says unknown rather than zero for a window with nothing in it", () => {
    const empty = scorecard({ since: at(59 * DAY), until: at(58 * DAY) });
  ...
  it("states what it cannot answer", () => {
    const limitations = card().limitations.join(" ");
