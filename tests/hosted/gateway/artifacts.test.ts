...
const artifact = await store.put(await writeBuiltTree(DATA_DIR), provenance("job-artifacts"));
...
/** An admitted call. */
const admitted = async (path: string, extra: Parameters<typeof call>[0] = {}) => call({ path, cookie: await sessionCookie(COOKIE), ...extra });
...
describe("the page", () => {
  it("serves index.html at the root, guarded and attributed", async () => {
    const { req, params } = await admitted("/");
  ...
  it("falls back to index.html for an app route with no extension", async () => {
    const { req, params } = await admitted("/requests/123");
    const res = await handleGateway(req, params);
    expect(res.status).toBe(200);
    expect(res.headers.get("content-type")).toBe("text/html; charset=utf-8");
    expect(await res.text()).toBe(BUILT_FILES["index.html"]);
...
describe("assets", () => {
  it("serves a hashed script with its own type and a cacheable policy", async () => {
    const { req, params } = await admitted("/assets/app-abc123.js");
    const res = await handleGateway(req, params);
    expect(res.status).toBe(200);
  ...
  it("does not let an unhashed asset be cached under a name that can change", async () => {
    const { req, params } = await admitted("/assets/logo.svg");
    const res = await handleGateway(req, params);
    expect(res.status).toBe(200);
  ...
  it("answers 404 for a file this release does not contain", async () => {
    const { req, params } = await admitted("/missing.js");
    const res = await handleGateway(req, params);
  ...
  it("answers 404 for a traversal, admitted or not", async () => {
    const { req, params } = await admitted("/x", { segments: ["..", "secret"] });
...
describe("HEAD", () => {
  it("answers the headers of the file and no body", async () => {
    const { req, params } = await admitted("/assets/app-abc123.js", { method: "HEAD" });
    const res = await handleGateway(req, params);
    expect(res.status).toBe(200);
    expect(res.headers.get("content-type")).toBe("text/javascript");
  ...
  it("is refused without a session, like any other read", async () => {
    const { req, params } = await call({ path: "/assets/app-abc123.js", method: "HEAD" });
    const res = await handleGateway(req, params);
...
describe("Range", () => {
  ...
  it("answers a single byte range with 206 and the exact slice", async () => {
    const { req, params } = await admitted("/assets/app-abc123.js", { headers: { range: "bytes=0-4" } });
    const res = await handleGateway(req, params);
  ...
  it("answers a suffix range from the end", async () => {
    const { req, params } = await admitted("/assets/app-abc123.js", { headers: { range: "bytes=-6" } });
    const res = await handleGateway(req, params);
    expect(res.status).toBe(206);
  ...
  it("answers 416 for a range past the end, and for a multi-range request", async () => {
    for (const range of [`bytes=${script.length + 10}-`, "bytes=0-2,5-7", "items=0-1", "bytes=-0"]) {
      const { req, params } = await admitted("/assets/app-abc123.js", { headers: { range } });
      const res = await handleGateway(req, params);
  ...
  it("is refused without a session", async () => {
    const { req, params } = await call({ path: "/assets/app-abc123.js", headers: { range: "bytes=0-4" } });
    const res = await handleGateway(req, params);
    expect(res.status).toBe(401);
    expect(gatewayTelemetry.artifactServed).toBe(0);
  });
...
describe("conditional requests", () => {
  ...
  it("accepts a weak tag and a list, and ignores a tag for other bytes", async () => {
    const etag = await scriptEtag();
    const weak = await admitted("/assets/app-abc123.js", { headers: { "if-none-match": `W/${etag}` } });
    expect((await handleGateway(weak.req, weak.params)).status).toBe(304);

    const list = await admitted("/assets/app-abc123.js", {
      headers: { "if-none-match": `"other", ${etag}` },
    });
    expect((await handleGateway(list.req, list.params)).status).toBe(304);

    const other = await admitted("/assets/app-abc123.js", { headers: { "if-none-match": '"nope"' } });
