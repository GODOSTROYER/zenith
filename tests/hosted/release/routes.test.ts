...
async function wire(holder: string = owner.id) {
  const runtime = await runtimeDouble({ store });
  const build = await buildRunnerDouble();
  const usage = await usageDouble();
  return release.setReleaseDepsForTests({
    ...
    activeGrant: async (appId, subject) => (subject === holder ? grant(appId, subject) : null),
  });
}
...
describe("the publish, job and release routes", () => {
  it("answers 202 with the queued job, then serves it back with its log", async () => {
    undo = await wire();
    const app = (await authority.authority().repos.apps.getBySlug("tracker"))!;
    const jobId = uuid();
  ...
  it("answers 202 for a repeated job id without queueing a second publish", async () => {
    ...
    expect((await authority.authority().repos.jobs.listByApp(app.id)).filter((j) => j.status === "queued")).toHaveLength(1);
  });
...
describe("POST /api/hosted/apps/[appId]/rollback", () => {
  it("answers 409 when the named release cannot be rolled back to", async () => {
    undo = await wire();
    const app = (await authority.authority().repos.apps.getBySlug("tracker"))!;
    const active = (await authority.authority().repos.apps.get(app.id))!.activeReleaseId!;
    const res = await call(
      rollbackRoute.POST,
