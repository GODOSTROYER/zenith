...
describe("build slots", () => {
  it("runs one job per app and two across the install, and queues the rest", async () => {
    ...
    expect(await h.release.claimJob(a1)).not.toBeNull();
    expect((await h.authority.authority().repos.jobs.get(a1))?.status).toBe("running");
    ...
    expect(await h.release.claimJob(a2)).toBeNull();
    expect((await h.authority.authority().repos.jobs.get(a2))?.status).toBe("queued");
    await expect(h.authority.authority().repos.jobs.claim(a2, "another-worker", 60_000)).rejects.toThrowError(
      /Another job is already running for this app/
    );
    expect((await h.authority.authority().repos.jobs.get(a2))?.status).toBe("queued");

    // A different app may run concurrently: that is the second pilot slot.
    expect((await h.release.buildSlot(apps["flight-b"])).ok).toBe(true);
    expect(await h.release.claimJob(b1)).not.toBeNull();
    expect(await h.authority.authority().repos.jobs.countRunning()).toBe(2);

    // The third app waits, and the refusal says why.
    const slot = await h.release.buildSlot(apps["flight-c"]);
    expect(slot.ok).toBe(false);
    expect(slot.reason).toContain("2 builds are already running");
    expect(await h.release.claimJob(c1)).toBeNull();
    // A tick with no room changes nothing.
    await h.release.tickJobs();
    expect((await h.authority.authority().repos.jobs.get(c1))?.status).toBe("queued");

    // Free a slot and the queued app moves.
    await h.authority.authority().repos.jobs.cancel(b1, "test finished with this slot");
    expect(await h.authority.authority().repos.jobs.countRunning()).toBe(1);
    expect((await h.release.buildSlot(apps["flight-c"])).ok).toBe(true);
    expect(await h.release.claimJob(c1)).not.toBeNull();
    expect((await h.authority.authority().repos.jobs.get(c1))?.status).toBe("running");
  ...
  it("refuses to claim a job that is not queued, naming its status", async () => {
    ...
    await h.authority.authority().repos.jobs.cancel(jobId, "cancelled before it ran");
...
describe("the ticker", () => {
  it("starts and stops idempotently and never holds the process open", async () => {
    expect(await h.release.hostedJobRunnerRunning()).toBe(false);
    await h.release.startHostedJobRunner();
    await h.release.startHostedJobRunner();
    expect(await h.release.hostedJobRunnerRunning()).toBe(true);
    await h.release.stopHostedJobRunner();
    await h.release.stopHostedJobRunner();
    expect(await h.release.hostedJobRunnerRunning()).toBe(false);
  });
