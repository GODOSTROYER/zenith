 ...
 * LocalStack only simulates, a resource Orrery does not own, a service and a
 * route.
 */
const base = (): Manifest => ({
  version: 1,
...
describe("localstack teardown planning", () => {
  ...
  it("never deletes a resource Orrery does not own", () => {
    // `referenced` means it exists in the cloud and Orrery only reads it.
...
describe("aws preview access copy", () => {
  it("claims no assumed role and no read it cannot perform", async () => {
    ...
    expect(copy).not.toMatch(/You create an IAM role|trust Orrery's principal/i);
    ...
    // The plan preview must not imply a role Orrery assumes either.
